...
## Sprint 3 (done) — Business URL Analysis, not the agents originally planned here

**Correction to note (2026-08-07, Sprint 3 closure):** everything below this line, in this section, is what this document predicted Sprint 3 would be, written at the end of Sprint 2. That is **not** what Sprint 3 actually built. The Founder Directive issued after the Sprint 2 Architecture Review Gate instead scoped Sprint 3 to the Analysis Engine — a business URL in, a customer-presentable Opportunity Report out — deliberately ahead of Discovery/Qualification/Research, on the reasoning that a report a founder can actually look at and trust is a more direct test of the evidence-first architecture than an agent whose output nobody reviews yet. See `docs/SPRINT_STATUS.md`'s Sprint 3 section and `docs/SPRINT_3_REVIEW.md` for what was actually built. Same pattern as this document's own Sprint 1→2 correction above: a sequencing change, not a silent scope cut, recorded rather than quietly overwritten. The Discovery/Qualification/Research scope described below has not been built and is not this document's current Sprint 4 prediction either — see `docs/SPRINT_4_DESIGN_REVIEW.md`, which scopes Sprint 4 to website generation (the Design stage) instead, per `docs/ARCHITECTURE_DECISIONS.md` ADR-010's pipeline ordering. Left in place below as a historical record of what was planned, not a current roadmap.

**Scope, as originally planned (superseded — see correction above): Discovery Engine, Opportunity Scoring, and Research Engine** — the first agents that actually publish real events through the Sprint 2 event bus, rather than the event catalog existing with nothing producing most of it.
