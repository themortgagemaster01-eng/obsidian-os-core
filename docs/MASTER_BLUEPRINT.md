...
**Obsidian OS is an Autonomous Client Acquisition Operating System.** That precise phrase is deliberate and should be used over looser alternatives: not "an AI Agency Operating System" in the vague, category-noun sense (accurate but underspecified), and absolutely not a website builder with some automation bolted on. The product's job is to run an entire client-acquisition pipeline autonomously, end to end, for a small digital agency:

**Discovery → Qualification → Research → Design → Proposal → Email → CRM → Learning → Analytics**

A redesigned website is one artifact the *Design* stage produces — website generation is one factory inside this pipeline, not the pipeline's identity. Two stages in that list map onto capability this document names elsewhere, and the mapping is worth stating explicitly so this document doesn't contradict itself across sections: **Qualification** is the Opportunity Scoring Agent (§4); **Learning** is the Decision Memory layer (§4, §6). §3's "independent-subsystems model" describes the same pipeline again, in engineering-subsystem vocabulary rather than product-pipeline vocabulary — see the reconciliation note there. Anyone documenting, pitching, or extending Obsidian OS who reduces it to "a tool that builds websites," or even to a generic "AI agency operating system" without the acquisition-pipeline specificity, has misunderstood the product. See `docs/VISION_GUARDRAILS.md` for the explicit, checkable boundary of what Obsidian OS never becomes and always is — every future feature proposal should be checked against that document before it's built.
...
**Architecture Principles (non-negotiable).** These are enforced constraints, not style preferences — a code change or feature proposal that violates one of these needs to either be reshaped or explicitly justified in a new `docs/ARCHITECTURE_DECISIONS.md` entry, the same way a violation of `docs/VISION_GUARDRAILS.md` would be. See ADR-000 for the founding premise these are derived from.

1. **The Mission Engine owns workflows.** State transitions happen in exactly one place — `lib/workflow/mission-workflow.ts::transitionMissionState()` and its named wrappers (`rejectMission()`, `archiveMission()`) — never as an inline `.update()` call from a component, route handler, or service that isn't the Mission Engine itself. See `docs/MISSION_ENGINE.md`.
2. **React never owns business logic.** Components (`app/`, `components/`) render and handle user interaction; they do not decide what a valid state transition is, compute derived business values, or talk to Supabase directly for anything beyond the one sanctioned Route Handler mutation path. If a component needs to make a decision more complex than "which prop do I pass," that decision belongs in `lib/services`.
3. **Business logic belongs in services.** `lib/services` is the only layer allowed to orchestrate across multiple repositories, or to coordinate a repository with the workflow engine or event bus. Repositories (`lib/repositories`) stay mechanical — one file per table, no business rules.
4. **Third-party APIs are isolated through adapters.** No component, service, or route handler calls an external API (an LLM provider, a crawling/analysis service, a future email-sending provider) directly. Each integration gets a dedicated adapter module so the rest of the codebase depends on a stable internal interface, not a specific vendor's SDK — the same port/adapter shape the Event Bus already uses (`docs/ARCHITECTURE_DECISIONS.md` ADR-006).
5. **Every workflow emits events.** Any code that changes a mission's state, or that represents a meaningful unit of agent or human work completing, publishes a typed `DomainEvent` through `EventBus.publish()` (`lib/events/`). A workflow that silently mutates data without publishing an event is incomplete, even if the mutation itself is correct.
6. **Human approval is required before outreach — always.** No code path is permitted to send an email, publish a live site, or take any client-visible action without an explicit human approval step recorded first. This is `docs/VISION_GUARDRAILS.md`'s single non-negotiable trust boundary, restated here as an architectural constraint rather than a product principle: it must be enforced in code, not just honored in intent.
7. **Documentation changes whenever architecture changes.** `docs/MASTER_BLUEPRINT.md` and `docs/ARCHITECTURE_DECISIONS.md` are updated in the same change as the code that alters what they describe, not in a follow-up pass. A pull request (or sprint) that changes architecture without a corresponding doc update is incomplete, per `docs/10-Development-Standards.md`.
8. **Every sprint produces customer-visible value.** Per the Founder Directive issued after the Sprint 2 Architecture Review Gate: no infrastructure-only sprint should exist unless absolutely necessary, and every feature built should satisfy at least one of — delivers customer value, removes meaningful technical debt, or improves customer experience — otherwise it's deferred. Progress is measured against proximity to a first paying customer, not lines of code or component count. Sprint 2 was the last sprint permitted to be infrastructure-only; every sprint from Sprint 3 onward ships something a customer could actually see or use.
...
**Reconciling this list with §1's product-pipeline naming.** §1 describes the same end-to-end flow in product vocabulary (Discovery → Qualification → Research → Design → Proposal → Email → CRM → Learning → Analytics); this section describes it in engineering-subsystem vocabulary. They are one pipeline, named twice for two audiences, not two different pipelines — the mapping: Discovery Engine = Discovery, the Opportunity Scoring Agent = Qualification, Research Engine = Research, Design Engine = Design (website generation is one factory *within* this stage, not the whole stage), Proposal Engine = Proposal, Outreach Engine = Email, CRM = CRM, the Decision Memory layer = Learning, Analytics = Analytics. Approval Queue and Mission Control sit alongside this pipeline rather than inside it — they're where a human intervenes in and observes the pipeline, not a stage the pipeline passes through.

---

## 4. AI Systems

**Sprint 2 status: zero agents are implemented.** This section is forward-looking specification, not a description of running code — say this plainly to anyone reading it, because the event catalog and database types already have the *shape* of agent output (e.g. `WebsiteScannedPayload`, `SEOCompletePayload` in `lib/events/types.ts`), which can look deceptively like the agents exist. They don't. Nothing publishes those event types today except the workflow engine's own `MissionStarted`, `StateChanged`, `MissionRejected`, and `MissionArchived` events, and the Decision Memory layer's `DecisionLogged`.
...
Plus the **Decision Memory layer**, which is not an agent that acts but a passive learner: `lib/services/decision-service.ts::logDecision()` records every human decision (approve/reject/edit and its context) as training data from day one, with no ML or scoring logic anywhere yet. See `docs/04-AI-Systems.md` for each agent's intended input/output contract against the event bus, and for the explicit statement of what "implement this agent" will require: an LLM call (Anthropic primary, per `.env.example`'s `ANTHROPIC_API_KEY`), a failure/retry policy (none exists yet — no agent runtime exists at all), and a publish call through `EventBus.publish()` rather than a direct `mission_events` insert.

---

## 5. Mission Control

Mission Control (`app/page.tsx`) is the one user-facing screen that exists. It's a server component: it resolves the signed-in user's `default_organization_id` via `profileRepository.findById()`, fetches that organization's missions via `listMissionsForOrganization()` and completed designs via `websiteDesignRepository.listCompletedByOrganization()`, and computes five real stats via `computeMissionControlStats()` — Active Missions (state not in `sent`/`archived`/`rejected`), Waiting Approval (state is `reviewing`, the Founder Approval Gate — fixed by the Dashboard Product Pass, which had been querying the later, distinct `approval` state and so under-reported), QA Ready (state is `qa`), Preview Ready (a completed `website_designs` row exists), and Completed Today (state is `sent` and `state_changed_at` falls within today). The Dashboard Product Pass removed the five earlier CRM-style placeholder tiles (Revenue Pipeline, Meetings Scheduled, Proposal Queue, Draft Emails, Website Builds) — Mission Control shows only capabilities the pipeline actually has today; a future capability earns a tile once it has real backing data, not before.

Below the stats, `MissionList` (`components/mission-control/mission-list.tsx`) renders each mission's business name, URL, creation date, and a `StateBadge` (`components/mission-control/state-badge.tsx`) color-coded by pipeline position, or an honest empty state ("No missions yet") when there are none. Missions at `reviewing` are sorted first and given a distinct card treatment so the ones needing a founder decision are immediately obvious; a mission with a completed Design Preview gets a direct link to the existing `/missions/[id]/preview` route. `NewMissionDialog` (styled as the dashboard's dominant CTA) posts to `POST /api/missions` and refreshes the list — see §7.
...
**Sprint 2 (this sprint) — done.** Unified `status`+`stage` into one canonical `state` field (11 states, text+CHECK, not a native enum, with old→new backfill logic — see the ADR log for why); multi-tenancy groundwork (`organizations`, `organization_members`, auto-provisioned personal org, org-scoped RLS everywhere); a formal typed event bus (`lib/events/`) as the mission timeline's single writer, replacing direct `mission_events` inserts; the Decision Memory schema + `logDecision()` service (architecture only, no ML); the Memory Vault (`companies` table) with `findOrCreateCompany()` wired into mission creation so it starts accumulating data immediately; UI updated to match the new schema.
