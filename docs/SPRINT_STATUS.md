...
**Item 1 — LLM-powered Design Intelligence: blocked, not started.** This repository has never called an AI provider (confirmed again: `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are both present but empty in `.env.local`/`.env.example`). Per explicit instruction, no call was fabricated and the deterministic Phase 2 services (`design-brief-service.ts`, `design-generation-service.ts`) were left as-is rather than silently kept as a permanent substitute. Recommendation delivered to the founder for a decision: Anthropic Claude as primary (already named "primary" in this repo's own `.env.example` comment since Sprint 1), `ANTHROPIC_API_KEY` env var, standard Anthropic API access with billing enabled (no special tier). Once a key is available, replacing `buildDesignBrief()`'s deterministic core with a model call — while keeping Generation fully deterministic per the spec's separation-of-responsibilities principle — is the next unblocked step.

111 tests total (19 new since Phase 2). `npm test` and `npm run build` both pass clean. No further items from this spec are authorized beyond 1–3.
