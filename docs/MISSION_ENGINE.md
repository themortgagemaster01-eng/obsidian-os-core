...
**Reality:** Fully built and exercised by the one real caller today — the "New Mission" flow only ever calls `createMission()`, which sets the initial `discovered` state. No UI currently calls `transitionMissionState()`, `rejectMission()`, or `archiveMission()` — they exist, are correct, and are unit-testable in isolation, but nothing in the running application invokes them yet. This is the single largest gap between "the engine is built" and "the engine is used."
...
The event catalog (`lib/events/types.ts`) is a fixed, typed, discriminated union of 10 event types: `MissionStarted`, `WebsiteScanned`, `SEOComplete`, `ProposalReady`, `EmailDraftReady`, `MissionApproved`, `MissionRejected`, `MissionArchived`, `StateChanged`, `DecisionLogged`. This union is kept string-for-string in sync with the `mission_events.event_type` `CHECK` constraint — adding an event type requires updating both.

**Reality, split honestly by event type:**
- **Fully built and actively published today:** `MissionStarted` (on mission creation), `StateChanged` (on every state transition), `MissionRejected`, `MissionArchived` — all four are published by `lib/workflow/mission-workflow.ts` today, on every request that exercises those code paths.
- **Defined but never published by anything:** `WebsiteScanned`, `SEOComplete`, `ProposalReady`, `EmailDraftReady`, `MissionApproved`, `DecisionLogged`. These exist as types with a matching database constraint value, ready for an agent or an Approval Queue UI to publish them, but zero code in the repository publishes any of them today. They are schema and type-system readiness, not working features.
- **Durability:** the in-process fan-out to subscribers is explicitly non-durable — it does not survive a process restart and does not cross server instances. This is acceptable today because nothing subscribes to it yet, and is flagged as something Sprint 3's job runner (§6) must address before any agent depends on cross-process delivery (`docs/ARCHITECTURE_DECISIONS.md` ADR-006).
...
**Reality: does not exist.** There is no background worker, no job queue, no scheduler, and no concept of a process running independently of an HTTP request. Every piece of Mission Engine code that runs today runs synchronously inside a request (either a Server Component render or the `POST /api/missions` Route Handler). "Nightly pipeline work" is described in `docs/MASTER_BLUEPRINT.md` §2 as part of the intended user journey, but nothing in the codebase schedules or executes it. This is the most consequential piece of unbuilt infrastructure standing between the current codebase and any autonomous agent actually doing work — an agent needs somewhere to run that isn't "inside a page load."
...
## 10. Summary: what a Sprint 3 agent actually needs from this engine

Any future agent (Discovery, Research, Design, Proposal, Outreach, Opportunity Scoring) integrates with the Mission Engine through exactly two surfaces, and should not need a third: call `transitionMissionState()` to move a mission forward, and call `EventBus.publish()` with a typed `DomainEvent` to record what it did. Both interfaces already exist and are stable. What does not yet exist, and what Sprint 3 must build before "an agent does real work unattended" is true, is: somewhere for that agent's code to run outside a request (§6), a retry/failure policy for when it fails (§5), and a queue for it to pull work from (§7). Building an agent without first deciding those three things would mean building on top of a gap, not a foundation.
