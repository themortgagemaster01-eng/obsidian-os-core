...
**This file must be updated every future sprint, alongside `docs/MASTER_BLUEPRINT.md`.** A sprint that changes the architecture without a corresponding ADR entry here is incomplete, per the documentation-first policy in `docs/10-Development-Standards.md`. Entries are chronological, oldest first. Each entry: title, date, status, context, decision, consequences/alternatives considered. **ADR-000 records the product philosophy every later entry serves — every ADR from ADR-011 onward should reference it explicitly wherever its decision trades off against, or is justified by, that founding premise.**

---

## ADR-000: Product Philosophy — why Obsidian OS exists (retroactive)

**Date:** 2026-08-05 (recorded retroactively, per the Founder Directive issued after the Sprint 2 Architecture Review Gate)
**Status:** Accepted

**Context.** Every ADR from ADR-001 onward documents a technical decision made in service of *something* — but that something was never itself written down as a first-class entry in this log. It lived only in `docs/MASTER_BLUEPRINT.md` §1 and `docs/VISION_GUARDRAILS.md`, both of which are living documents that can be revised. This log, by contrast, is meant to be an immutable record of reasoning at a point in time. Without a philosophy entry at the top of it, a future engineer reading ADR-001 through ADR-010 could reconstruct *what* was decided but not *why any of it was worth deciding* — the load-bearing premise underneath every subsequent trade-off. This gap became visible during the Sprint 2 Architecture Review Gate, when an external review and a founder directive both needed to reference "why does this product exist" as settled context rather than re-litigate it.

**Decision.** Record, as ADR-000, the founding premise every later ADR implicitly depends on: **Obsidian OS exists to compress the analytical and creative labor of client acquisition for a small digital agency into a five-minute human review**, not to automate any single artifact (a website, an email, a proposal) in isolation. The product is the *pipeline* — Discovery → Qualification → Research → Design → Proposal → Email → CRM → Learning → Analytics (`docs/MASTER_BLUEPRINT.md` §1) — and every subsystem ADR-001 onward exists to make that pipeline durable, evolvable, and trustworthy enough that a human can safely delegate the labor without delegating the judgment. Three commitments follow directly from this premise and are treated as constraints on every future decision, not aspirations: (1) the system prepares, it never unilaterally acts on a prospect or client — human approval before outreach is non-negotiable; (2) architecture is written down before or alongside the code that implements it, because a pipeline meant to run autonomously for years cannot depend on any one person's memory of why it works the way it does; (3) every sprint is measured against whether it moved the product toward a human being willing to trust it with real client-facing stakes, not against lines of code, component count, or infrastructure elegance for its own sake.

**Consequences / how this governs later ADRs.** Every ADR from ADR-001 forward should be read as an instance of this premise applied to a specific technical question, and **every ADR from this point forward (ADR-011 onward) should explicitly reference ADR-000** when its decision trades off against, or is justified by, the product philosophy stated here — the same way a legal opinion cites the constitutional clause it's interpreting, not just the precedent case. Where a future decision appears to conflict with ADR-000 (for instance, a proposed feature that would auto-send something without human approval, or an infrastructure investment with no path to customer-visible value), that conflict should be resolved in this log explicitly, not silently — either the feature is rejected, or ADR-000 itself is revised with a new dated entry explaining why the founding premise changed. A silent violation of ADR-000 anywhere in the codebase should be treated as a bug, not a judgment call.
...
## ADR-010: Product framing refined to "Autonomous Client Acquisition Operating System" with an explicit named pipeline

**Date:** 2026-08-05
**Status:** Accepted

**Context.** During the Sprint 2 Architecture Review Gate, it became clear that "AI Agency Operating System" — the framing used throughout Sprint 1 and Sprint 2 docs, and still printed in the Sprint 1 login page UI copy — is accurate but underspecified: it names a category without naming the actual mechanism (a pipeline) or making the anti-goal (a website builder) impossible to miss. The product needed a framing precise enough that "does this feature belong" is a checkable question, not a judgment call, and a docs sweep found the old framing duplicated near-verbatim in three places (`docs/MASTER_BLUEPRINT.md` §1, `docs/00-Executive-Summary.md`, `docs/01-Product-Vision.md`) that could each drift independently if only one were updated.

**Decision.** Adopt "Autonomous Client Acquisition Operating System" as the precise product framing, with an explicit nine-stage pipeline named at the product level: **Discovery → Qualification → Research → Design → Proposal → Email → CRM → Learning → Analytics**. Website generation is named explicitly as one factory *within* the Design stage, not the product's identity. Two stages reconcile with existing vocabulary rather than introducing new unbuilt concepts: Qualification = the already-specified Opportunity Scoring Agent; Learning = the already-specified Decision Memory layer. `docs/MASTER_BLUEPRINT.md` §3's existing "independent-subsystems model" (Discovery Engine → Research Engine → Design Engine → Proposal Engine → Outreach Engine → CRM → Analytics → Approval Queue → Mission Control) is kept as-is and explicitly reconciled against the new product-pipeline naming with a mapping table, rather than replaced — one pipeline, two vocabularies for two audiences (engineering subsystem boundaries vs. product narrative), deliberately kept distinct rather than merged into a single naming scheme that would serve neither audience well. `docs/MASTER_BLUEPRINT.md` §1, `docs/00-Executive-Summary.md`, and `docs/01-Product-Vision.md` were all updated in the same pass so the framing can't drift between them. A new `docs/VISION_GUARDRAILS.md` was created alongside this ADR as the canonical, checkable boundary of what Obsidian OS never becomes and always is — see that document for the actual guardrail list.

**Consequences / alternatives considered.** A full mechanical find-and-replace of "AI Agency Operating System" everywhere it appears was explicitly rejected — it would have touched `app/login/page.tsx` and `app/layout.tsx`'s metadata description, both of which are application code, and this ADR was written during an Architecture Review Gate in which no code changes are in scope (build/merge/Sprint 3 are all paused pending independent review). This is a real, acknowledged inconsistency left in place on purpose: **the running application's UI still displays the old "AI Agency Operating System" tagline** (`app/login/page.tsx` line ~72, `app/layout.tsx`'s `metadata.description`) while the documentation now uses the refined framing. This is tracked as a concrete, named follow-up for the first code-touching sprint after this review concludes, not silently left as drift — see `docs/SPRINT_2_REVIEW.md`'s Technical Debt section. The alternative of *not* refining the framing at all (leaving "AI Agency Operating System" as-is, relying on the existing "not a website builder" disclaimer to do the anti-drift work) was rejected because a category label plus a single negative disclaimer is weaker product-drift protection than a named pipeline plus a dedicated guardrails document — the latter gives a concrete test ("is this stage in the pipeline, and does it belong to an existing stage's scope?") rather than relying on a reviewer's judgment call each time a new feature is proposed.
